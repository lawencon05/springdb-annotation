...
public class MahasiswaDaoHibernateNQImpl extends BaseDaoImpl implements MahasiswaDao {
	...
	public void insert(Mahasiswa data) throws Exception {
		em.createNativeQuery(" INSERT INTO mahasiswa (nim, nama, univ_id, waktu_masuk) VALUES (?1, ?2, ?3, ?4) ")
	...
	public void insertReturnId(Mahasiswa data) throws Exception {
		List<?> listResult = em.createNativeQuery(
				" INSERT INTO mahasiswa (nim, nama, univ_id, waktu_masuk) VALUES (?1, ?2, ?3, ?4) RETURNING id ")
				.setParameter(1, data.getNim()).setParameter(2, data.getNama())
				.setParameter(3, data.getUniversitas().getId()).setParameter(4, data.getWaktuMasuk()).getResultList();
	...
	public void update(Mahasiswa data) throws Exception {
		em.createNativeQuery(" UPDATE mahasiswa SET nim = ?1, nama = ?2, univ_id = ?3 WHERE id = ?4  ")
				.setParameter(1, data.getNim()).setParameter(2, data.getNama())
	...
	public void updateCustom(Mahasiswa data) throws Exception {
		em.createNativeQuery(" UPDATE mahasiswa set nama = ?1 WHERE id = ?2 ").setParameter(1, data.getNama())
				.setParameter(2, data.getId()).executeUpdate();
	}

	@Override
	public void delete(Mahasiswa data) throws Exception {
		em.createNativeQuery(" DELETE FROM mahasiswa WHERE id = ?1 ").setParameter(1, data.getId()).executeUpdate();
	}

	@Override
	public Mahasiswa getMhsById(Long id) throws Exception {
		List<?> listResult = em.createNativeQuery("SELECT * from mahasiswa where id = ?1 ").setParameter(1, id)
				.getResultList();

		return listResult.size() > 0 ? (Mahasiswa) listResult.get(0) : null;
	}

	@Override
	public List<Mahasiswa> getAllMhs() throws Exception {
		List<Mahasiswa> listMhs = new ArrayList<>();
		List<?> listObj = em.createNativeQuery("SELECT id, nim, nama, univ_id, waktu_masuk FROM mahasiswa")
				.getResultList();
		listObj.forEach(val -> {
			...
			mhs.setWaktuMasuk(((Date) objArr[4]).toLocalDate());
			listMhs.add(mhs);
		});
		return listMhs;
	}
